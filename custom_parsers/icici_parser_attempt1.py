    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            for line in lines:
                tokens = line.split()
                if len(tokens) < 6:
                    print(f"Skipping line with fewer than 6 tokens: {line}")
                    continue
                if any(token in line for token in ['Date', 'Description', 'Credit', 'Debit']):
                    print(f"Skipping line with header-like words: {line}")
                    continue
                ...
                if len(float_tokens) < 3:
                    print(f"Skipping line with fewer than 3 float tokens: {line}")
                    continue
